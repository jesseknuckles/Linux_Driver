...
//Defining task_info
typedef struct {
	unsigned int __state; // Task state (0 means running)
	unsigned int cpu; // CPU running this task (from thread_info)
	int prio; // Priority
	pid_t pid; // PID
	pid_t tgid; // TGID
...
#define SCULL_IOCIQUANTUM _IOR(SCULL_IOC_MAGIC, 7, task_info) //defining SCULL_IOCIQUANTUM syscall
